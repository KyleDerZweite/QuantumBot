package de.quantum.modules.audit.listener.emoji;

import de.quantum.core.events.EventAnnotation;
import de.quantum.core.events.EventInterface;

import de.quantum.modules.audit.AuditHandler;
import net.dv8tion.jda.api.entities.Guild;
import net.dv8tion.jda.api.events.emoji.update.*;

@EventAnnotation({
        EmojiUpdateNameEvent.class,
        EmojiUpdateRolesEvent.class
})
public class EmojiUpdateListener implements EventInterface<GenericEmojiUpdateEvent<?>> {

    @Override
    public void perform(GenericEmojiUpdateEvent<?> event) {
        Guild guild = event.getGuild();
        if (!AuditHandler.isGuildLogging(guild, event.getClass())) {
            return;
        }

        System.out.println(event.getClass().getName());
    }
}
